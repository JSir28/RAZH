...
def get_args_parser():
    ...
    parser.add_argument('--gamm', default=1, type=float)
    ...
    # Dataset parameters
    parser.add_argument('--data_path', default='/mnt/f88fa63a-2225-40fb-9afa-99a7c125ae28/jy/datasets/cifar-10/train_cifar10/', type=str,
                        help='dataset path')
    ...
    parser.add_argument('--TOP_K', default=[5000], type=int,
                        help='TOP_K')
    parser.add_argument('--w2vpath', default='./word2vec/cifar10_word2vec_300d.npy',
                        type=str,
                        help='dataset')
...
def main(args):
    ...
    args.log_dir = args.log_dir +"/" + str(args.mask_ratio) + "/240521/" +str(time.time())+"_"+str(args.hash_length) + "_" + str(args.dataset_type)+ "_" + str(args.alpha)+ "_" + str(args.gamm)
    ...
    src_att = torch.tensor(np.load(args.w2vpath))
    src_att.to(device)

    # lab_att = read_attr(os.path.join(args.data_path, 'attributes/class_attribute_labels_continuous.txt')) #load class attribute
    ...
    data_loader_train, num_train = get_cifar10_train_data(args)
    test_loader, database_loader, num_test, num_database = get_cifar10_query_data(args, data_loader_train)
    print(num_test)
    print(num_database)

    # define the model
    model = models_mae_cls.__dict__[args.model](norm_pix_loss=args.norm_pix_loss,hash_length=args.hash_length,alpha=args.alpha,gamm=args.gamm,num_classes=args.classnum)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch%5==0 or  epoch + 1 == args.epochs:
            loss, Map= validata(args, test_loader, database_loader, num_test, num_database, src_att, lab_att, model, epoch,
                                                                   max_map, log_writer=log_writer)
            max_map = max(max_map, Map)
            print(f'Max map:{max_map:.6f}')
