...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
                 embed_dim=768, depth=24, num_heads=16, hash_length=64,
                 decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16, w2v_dim=300,
                 mlp_ratio=4., norm_layer=nn.LayerNorm, norm_pix_loss=False, num_classes=10, attr=None,alpha=0.0,gamm=0.0,
                 unseen_classes=None):
        super().__init__()
        self.alpha = alpha
        self.gamm = gamm
        ...
        if hash_length<64:
            ...
            self.embed_attr = nn.Sequential(nn.Linear(w2v_dim, w2v_dim),nn.Linear(w2v_dim, w2v_dim),nn.Linear(w2v_dim, w2v_dim))
    ...
    def forward_attribute(self, x, attr, lab_att, labels):
        ...
        for i in range(x_nor.shape[0]):
            cluster_ids_x, cluster_centers = kmeans(
                X=x_nor[i], num_clusters=2, distance='cosine', device=torch.device('cuda:0')
            )
    ...
    def forward_loss(self, imgs, pred,att_pred,mask, labels, cls_out, hash_out, cls_out_attr, hash_out_attr):
        ...
        hash_loss, quanti_loss, cls_loss, loss1_attr = criterion(cls_out_attr, hash_out_attr,labels)
        ...
        return loss1+ rec_loss+loss1_attr
...
def mae_vit_base_patch16_dec512d8b(**kwargs):
    w2v_att = torch.from_numpy(numpy.load('./word2vec/cifar10_word2vec_300d.npy'))  # loading attribute w2v
