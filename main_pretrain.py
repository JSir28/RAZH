...
def get_args_parser():
    ...
    parser.add_argument('--TOP_K', default=[5000], type=int,
                        help='TOP_K')
    parser.add_argument('--w2vpath', default='/mnt/f88fa63a-2225-40fb-9afa-99a7c125ae28/jy/mae_jy_cifar10/word2vec/cifar10_word2vec_300d.npy',
                        type=str,
                        help='dataset')
...
def main(args):
    ...
    args.log_dir = args.log_dir +"/" + str(args.mask_ratio) + "/" +str(time.time())+"_"+str(args.hash_length) + "_" + str(args.dataset_type)+ "_" + str(args.alpha)+ "_" + str(args.gamm)
    ...
    src_att = torch.from_numpy(np.load(args.w2vpath)) #loading attribute w2v
    path = os.path.join(args.data_path, "classes.txt")
    lab_att = read_attr(path) #load class attribute
    # lab_att = [i[0].split(" ") for i in lab_att]
    lab_att = [list(map(int,j)) for j in []]
