...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward_encoder(self, x, src_att, lab_att, labels, mode, mask_ratio):  # 图片 掩码率
        ...
        if mode == "train":
            x_masked, x_masked1, mask, ids_restore, mask1, ids_restore1 = self.random_masking(x, src_att, lab_att, labels, mode, mask_ratio)
        else:
            x_masked1 = None
            x_masked = x
    ...
    def forward_loss(self, imgs, pred,att_pred,mask, labels, cls_out, hash_out, mask1, ids_restore1, cls_out1, hash_out1):
        """
        imgs: [N, 3, H, W]
        pred: [N, L, p*p*3]
        ...
        _, _, _, loss1_attr = criterion(hash_out1,cls_out1,labels)
        # hash1 = torch.sign(hash_out_attr)
        # hash2 = torch.sign(hash_out)
        # loss3 =(hash_out - hash_out_attr).pow(2).mean()
        loss3 = torch.abs(torch.sum(hash_out1 - hash_out)/ hash_out.shape[0])
        ...
        loss2 = (loss2 * mask1).sum() / mask1.sum()  # mean loss on removed patches
    ...
    def forward(self,labels, lab_att,imgs, mode,mask_ratio=0.25):
        ...
        latent, mask, ids_restore, cls_out, hash_out, latent_attr, mask1, ids_restore1, cls_out1, hash_out1 = self.forward_encoder(imgs, attr, lab_att, labels, mode,mask_ratio)
