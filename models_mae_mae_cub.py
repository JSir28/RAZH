...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
                 ...
                 mlp_ratio=4., norm_layer=nn.LayerNorm, norm_pix_loss=False, num_classes=200, attr=None,
                 unseen_classes=None):
        super().__init__()
    ...
    def random_masking(self, x, labels, mode, mask_ratio):
        """
    ...
    def forward_encoder(self, x, labels, mode, mask_ratio):  # 图片 掩码率
        # embed patches
        ...
        x_masked, mask, ids_restore = self.random_masking(x, labels, mode, mask_ratio)
        # append cls token
        ...
        return x_masked, mask, ids_restore, cls_out, hash_out
    ...
    def forward_loss(self, imgs, pred,mask, labels, cls_out, hash_out):
        """
        imgs: [N, 3, H, W]
        pred: [N, L, p*p*3]
    ...
    def forward(self,labels,imgs, mode,mask_ratio=0.25):
        # attr = self.embed_attr(self.V.float())
        latent, mask, ids_restore, cls_out, hash_out = self.forward_encoder(imgs, labels, mode,mask_ratio)
        ...
        loss = self.forward_loss(imgs, pred, mask, labels,cls_out, hash_out)
