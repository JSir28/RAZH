...
from util.draw_confuse_matrix import draw_confusion_matrix
...
def get_args_parser():
    ...
    parser.add_argument('--gamm', default=1e-4, type=float)

    # Model parameters
    parser.add_argument('--model', default='mae_vit_base_patch16', type=str, metavar='MODEL',
                        help='Name of mod el to train')
    parser.add_argument('--retrain', default='./checkpoint/checkpoint.pth', type=str,
...
def main(args):
    ...
    args.log_dir = args.log_dir +"/pretrain/clu/" + str(args.mask_ratio) + "/241109/" +str(time.time())+"_"+str(args.hash_length) + "_" + str(args.dataset_type)+ "_" + str(args.alpha)+ "_" + str(args.gamm)
    ...
    # define the model
    model = models_params.__dict__[args.model](norm_pix_loss=args.norm_pix_loss,hash_length=args.hash_length,alpha=args.alpha,gamm=args.gamm)
    ...
    acc1, attr_acc1,  acc5, attr_acc5, loss, Map, Precision = validata(args, test_loader, database_loader, num_test, num_database, src_att, lab_att, model, 0,
    ...
    #         acc1, attr_acc1,  acc5, attr_acc5, loss, Map, Precision = validata(args, test_loader, database_loader, num_test, num_database, src_att, lab_att, model, epoch,
    #                                                                max_map, log_writer=log_writer)
    #         if Map>max_map:
    #             torch.save(model, "/mnt/f88fa63a-2225-40fb-9afa-99a7c125ae28/jy/code/RAZH_AWA2_clu4/checkpoint/" + str(args.hash_length) + "/checkpoint_0724" + str(Map) + ".pth")
    #             torch.save(model.state_dict(), "/mnt/f88fa63a-2225-40fb-9afa-99a7c125ae28/jy/code/RAZH_AWA2_clu4/checkpoint/" + str(args.hash_length) + "/checkpoint_param_0708" + str(Map) + ".pth")
...
@torch.no_grad()
def validata(args,test_loader, database_loader, num_test, num_database, src_att,lab_att,model, epoch,max_map, log_writer):

    y_gt=[]
    y_pred=[]
    classes = open('/mnt/f88fa63a-2225-40fb-9afa-99a7c125ae28/jy/datasets/AWA2/Animals_with_Attributes2/classes.txt').readlines()
    ...
    for sample in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            # stat(model, img)
            flops, params = profile(model, img.cuda())
            print('FLOPs = ' + str(flops/1000**3) + 'G')
            print('Params = ' + str(params/1000**2) + 'M')
            loss, _, _ ,hash_out,cls_out= model(target,lab_att,images, mask_ratio=args.mask_ratio)
        

        predict_np = np.argmax(cls_out.cpu().detach().numpy(), axis=-1)   # array([0,5,1,6,3,...],dtype=int64)
