...
def get_args_parser():
    ...
    parser.add_argument('--gamm', default=1e-3, type=float)
    parser.add_argument('--beta', default=1, type=float)
    # Model parameters
    parser.add_argument('--model', default='mae_vit_base_patch16', type=str, metavar='MODEL',
                        help='Name of mod el to train')
    
    parser.add_argument('--threthold', default=0.1, type=float, 
                        help='threthold of replace')
    parser.add_argument('--clu_num', default=9, type=int, 
                        help='num of cluster')
...
def main(args):
    ...
    args.log_dir = args.log_dir +"/pretrain/params/" + str(args.mask_ratio) + "/2412/" +str(time.time())+"_"+str(args.hash_length) + "_" + str(args.dataset_type)+ "_" + str(args.alpha)+ "_" + str(args.gamm)
    ...
    # define the model
    model = models_clu.__dict__[args.model](norm_pix_loss=args.norm_pix_loss,clu_num=args.clu_num,hash_length=args.hash_length,alpha=args.alpha,beta=args.beta,gamm=args.gamm,threthold=args.threthold)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch%5==0 or  epoch + 1 == args.epochs:
            ...
            # if Map>max_map:
            #     torch.save(model, "/mnt/f88fa63a-2225-40fb-9afa-99a7c125ae28/jy/code/RAZH_AWA2_12/checkpoint/" + str(args.hash_length) + "/checkpoint_0724" + str(Map) + ".pth")
            #     torch.save(model.state_dict(), "/mnt/f88fa63a-2225-40fb-9afa-99a7c125ae28/jy/code/RAZH_AWA2_12/checkpoint/" + str(args.hash_length) + "/checkpoint_param_0708" + str(Map) + ".pth")
