...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward_encoder(self, x, src_att, lab_att, labels, mask_ratio):  # 图片 掩码率
        ...
        if self.training:
            x_masked, x_masked1, mask, ids_restore,mask1, ids_restore1 = self.random_masking(x, src_att, lab_att, labels, mask_ratio)
        else:
            x_attr = None
            x_masked = x
        ...
        #     cls_tokens = cls_token.expand(x_masked1.shape[0], -1, -1)
        #     x_attr = torch.cat((cls_tokens, x_masked1), dim=1)
        #     for blk in self.blocks:
        #         x_attr = blk(x_attr)
        ...
        hash_out1=None
        x_attr = None
        return x_masked, mask, ids_restore, cls_out, hash_out, x_attr, cls_out_attr, hash_out_attr,mask1,ids_restore1
    ...
    def forward_attribute(self, x, attr, lab_att, labels):
        ...
        x_clu = torch.randn(0, 196, 300).cuda()
        for i in range(x_nor.shape[0]):
            cluster_ids_x, cluster_centers = kmeans(
                X=x_nor[i], num_clusters=self.clu_num, distance='cosine', device=torch.device('cuda:0')
            )
    ...
    def forward_loss(self, imgs, pred,att_pred,mask,mask1, labels, cls_out, hash_out, cls_out_attr, hash_out_attr):
        """
        imgs: [N, 3, H, W]
        pred: [N, L, p*p*3]
        ...
        criterion = My_Loss(self.num_classes, self.hash_length, mixup_fn=None, smoothing=0,
                            alph=self.alpha, beta=1, gamm=1.0)
        ...
        # loss2 = (loss2 * mask1).sum() / mask.sum()  # mean loss on removed patches


        rec_loss = self.gamm*(loss+0.0+0.0)
        # loss3 = loss =loss2 =0.0
        return rec_loss+ loss1
    ...
    def forward(self,labels, lab_att,imgs,mask_ratio=0.25):
        ...
        latent, mask, ids_restore,cls_out, hash_out, latent_attr, cls_out_attr, hash_out_attr,mask1,ids_restore1 = self.forward_encoder(imgs, attr, lab_att,labels,mask_ratio)
        if self.training == False:
            return 0.0, None, mask, hash_out, cls_out
        # att_latent = self.forward_attribute(latent,lab_att,labels)
        # att_pred = self.forward_decoder(latent_attr, ids_restore1)
        pred = self.forward_decoder(latent, ids_restore)  # [N, L, p*p*3]
        loss = self.forward_loss(imgs, pred,None, mask,mask1, labels,cls_out, hash_out, cls_out_attr, hash_out_attr)
