...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,threthold=0.1,clu_num=9,
                 ...
                 mlp_ratio=4., norm_layer=nn.LayerNorm, norm_pix_loss=False, num_classes=50, attr=None,alpha=1e-1,gamm=1e-1,beta=1e-1,
                 unseen_classes=None):
        super().__init__()
        ...
        if hash_length==24 or hash_length==48:
            self.embed_attr = nn.Sequential(nn.Linear(w2v_dim, w2v_dim),nn.Linear(w2v_dim, w2v_dim))
        else:
            self.embed_attr = nn.Sequential(nn.Linear(w2v_dim, w2v_dim))
    ...
    def random_masking(self, x, src_att, lab_att, labels, mask_ratio):
        """
        ...
        noise1 = torch.rand(N, L, device=x.device)  # noise in [0, 1]
        ...
        x_, score = self.forward_attribute(x, attr=src_att, lab_att=lab_att, labels=labels)
        x_attr = torch.where(score>self.threthold, x_, x.to(torch.float16))
        x_attr = torch.gather(x_attr, dim=1, index=ids_restore.unsqueeze(-1).repeat(1, 1, D))
        ...
        # mask1 = torch.gather(mask, dim=1, index=ids_restore1)

        return x_masked, x_attr, mask, ids_restore
        # return x_masked, x_masked1, mask, ids_restore, mask1, ids_restore1

    

    def forward_encoder(self, x, src_att, lab_att, labels, mask_ratio):  # 图片 掩码率
        # embed patches
        ...
        x_masked, x_attr, mask, ids_restore = self.random_masking(x, src_att, lab_att, labels, mask_ratio)
        ...
        if self.training == True:
            ...
            # cls_tokens1 = cls_token.expand(x_masked1.shape[0], -1, -1)
            # x_masked1 = torch.cat((cls_tokens1, x_masked1), dim=1)
            # for blk in self.blocks:
            #     x_masked1 = blk(x_masked1)
            # x_masked1 = self.norm(x_masked1)
            # x_out1 = self.avgpool(x_masked1.transpose(1, 2))  # B C 1
            # x_out1 = torch.flatten(x_out1, 1)
            # hash_out1 = self.hash_layer(x_out1)
            # cls_out1 = self.head(hash_out1)
            # hash_out1 = torch.tanh(hash_out1)
        else:
            hash_out_attr = None
            cls_out_attr = None
            mask1 = None
        ...
        # return x_masked, mask, ids_restore, cls_out, hash_out, x_masked1, mask1, ids_restore1, cls_out1, hash_out1
    ...
    def forward_attribute(self, x, attr, lab_att, labels):

        attr_nor = (1. * attr / (torch.norm(attr, 2, -1, keepdim=True).expand_as(attr) + 1e-12)).cuda()
        # x = self.align_layer(x)  # align to w2v
        # x_nor = 1. * x / (torch.norm(x, 2, -1, keepdim=True).expand_as(x) + 1e-12)
        

        
        x_clu = torch.randn(0, 196, 768).cuda()
        all_cluster_ids = []
        for i in range(x.shape[0]):
            cluster_ids_x, cluster_centers = kmeans(
                X=x[i], num_clusters=25, distance='cosine', device=torch.device('cuda:0')
            )
            all_cluster_ids.append(cluster_ids_x.cpu().numpy())
            
            x_clu = torch.cat((x_clu, cluster_centers.cuda()[cluster_ids_x].unsqueeze(dim=0)), dim=0) 
        
        

        # 可视化每个样本
        p = self.patch_embed.patch_size[0]  # 每个patch的大小
        for i, cluster_ids in enumerate(all_cluster_ids):
            h = w = int(len(cluster_ids) ** 0.5)  # 假设patches构成正方形
            cluster_ids = cluster_ids.reshape(h, w)

            # 为每个聚类分配颜色
            unique_clusters = np.unique(cluster_ids)
            color_map = {cid: np.random.rand(3) for cid in unique_clusters}

            # 生成颜色图像
            colored_image = np.zeros((h * p, w * p, 3))  # 初始化为全黑图像
            for y in range(h):
                for x in range(w):
                    patch_color = color_map[cluster_ids[y, x]]
                    colored_image[y * p:(y + 1) * p, x * p:(x + 1) * p] = patch_color

            # 保存图片
            plt.imshow(colored_image)
            plt.axis('off')
            plt.title(f"Sample {i} Cluster Visualization")
            plt.savefig(f"/mnt/f88fa63a-2225-40fb-9afa-99a7c125ae28/jy/code/RAZH_AWA2_12/output_cluster/_sample_{i}.png")
        score = torch.einsum("abc, cd->abd", (x_clu, attr_nor.T.float()))
        ...
        result =  label_attr_sort_score[1]
        score = label_attr_sort_score[0]
        # result = label_attr_sort_score[1]
        # score = label_attr_sort_score[0]
        attr_decoder = attr[result.squeeze()]
        attr_decoder = attr_decoder.to(torch.float32)
        attr_decoder = self.unalign_layer(attr_decoder)
        return attr_decoder,score
    ...
    def forward_loss(self, imgs, pred,att_pred,mask, labels, cls_out, hash_out, cls_out_attr, hash_out_attr):
        ...
        criterion = My_Loss(self.num_classes, self.hash_length, mixup_fn=None, smoothing=0,
                            alph=self.alpha, beta=self.beta, gamm=1.0)
    ...
    def forward(self,labels, lab_att,imgs,mask_ratio=0.25):
        attr = self.embed_attr(self.V.float())
        # latent, mask, ids_restore, cls_out, hash_out, latent_attr, mask1, ids_restore1, cls_out1, hash_out1 = self.forward_encoder(imgs, attr, lab_att, labels,mask_ratio)
        latent, mask, ids_restore,cls_out, hash_out, latent_attr, cls_out_attr, hash_out_attr = self.forward_encoder(imgs, attr, lab_att,labels,mask_ratio)
        ...
        # loss = self.forward_loss(imgs, pred,att_pred, mask, labels,cls_out, hash_out, mask1, ids_restore1, cls_out1, hash_out1)
