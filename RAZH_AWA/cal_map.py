...
def compress(train, test, model, classes=10):
    retrievalB = list([])
    retrievalL = list([])
    for batch_step, (data, target, index) in enumerate(train):
        var_data = data.cuda()
        with torch.no_grad():
            _,_, code = model(var_data)
        retrievalB.extend(torch.sign(code).cpu().data.numpy())
        retrievalL.extend(target)

    queryB = list([])
    queryL = list([])
    for batch_step, (data, target,index) in enumerate(test):
        var_data = data.cuda()
        with torch.no_grad():
            _,_, code = model(var_data)
        queryB.extend(torch.sign(code).cpu().data.numpy())
...
def calculate_map(qB, rB, queryL, retrievalL):
    ...
    for iter in range(num_query):
        ...
        count = np.linspace(1, int(tsum), int(tsum)) # [1,2, tsum]
