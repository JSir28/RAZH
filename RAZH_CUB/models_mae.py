...
class MaskedAutoencoderViT(nn.Module):
    ...
    def random_masking(self, x, src_att, lab_att, labels, mask_ratio):
        ...
        x_attr, score = self.forward_attribute(x, attr=src_att, lab_att=lab_att, labels=labels)
        x_attr = torch.where(score>0.2, x_attr, x.to(torch.float16))

        # x_attr = torch.cat((x_masked, x_), dim=1)
        x_attr = torch.gather(x_attr, dim=1, index=ids_restore.unsqueeze(-1).repeat(1, 1, x_attr.shape[2]))
    ...
    def forward_attribute(self, x, attr, lab_att, labels):
        ...
        for i in range(x_nor.shape[0]):
            cluster_ids_x, cluster_centers = kmeans(
                X=x_nor[i], num_clusters=100, distance='cosine', device=torch.device('cuda:0')
            )
        ...
        attr_label[attr_label > 0] = 1
        attr_label[attr_label <= 0] = 0
        ...
        return attr_decoder,label_attr_sort_score[0]
