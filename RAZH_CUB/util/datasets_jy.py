...
def get_cifar10_train_data(args):
    ...
    dset_loaders = {}
    data_config = {
        "train_set": {"list_path": args.data_path + "/trainclasses.txt", "batch_size": args.batch_size},
        "database": {"list_path":args.data_path + "/classes.txt", "batch_size": args.batch_size},
        "test": {"list_path": args.data_path + "/testclasses.txt", "batch_size": args.batch_size}}
...
def get_cub_train_data(args):
    ...
    with open('/mnt/f88fa63a-2225-40fb-9afa-99a7c125ae28/jy/code/RAZH_CUB_clu/classes.txt', "w") as file:
        for item in classes:
            file.write(str(item) + "\n")
    # train_classes = open(data_config["train_set"]["list_path"]).readlines()
    # train_classes = [i.replace('\t', ' ').replace('\n', '').split(' ')[-1] for i in train_classes]
    ...
    unseen = [classes.index(i) for i in classes if i not in set_class]
    return dset_loaders["train_set"], len(dsets["train_set"]),unseen
