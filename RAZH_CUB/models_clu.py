...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
                 ...
                 mlp_ratio=4., norm_layer=nn.LayerNorm, norm_pix_loss=False, num_classes=200, attr=None, alpha=1,
                 gamm=1,
                 unseen_classes=None):
        super().__init__()
    ...
    def random_masking(self, x, src_att, lab_att, labels, mask_ratio):
        ...
        x_attr, score = self.forward_attribute(x_placed, attr=src_att, lab_att=lab_att, labels=labels)

        # x_attr = torch.cat((x_masked, x_), dim=1)
        # x_attr = torch.where(score>0.1, x_, x.to(torch.float16))
        x_attr = torch.gather(x_attr, dim=1, index=ids_shuffle.unsqueeze(-1).repeat(1, 1, x_attr.shape[2]))
    ...
    def forward_attribute(self, x, attr, lab_att, labels):
        ...
        #         X=x_nor[i], num_clusters=196, distance='cosine', device=torch.device('cuda:0')
        #     )
        #     # cluster_ids_x = torch.randint(0, 8, (196,)).cuda()
        #     # cluster_centers= torch.randn(9,300)
    ...
    def forward_loss(self, imgs, pred, att_pred, mask, labels, cls_out, hash_out, cls_out_attr, hash_out_attr):
        ...
        criterion = My_Loss(self.num_classes, self.hash_length, mixup_fn=None, smoothing=0,
                            alpha=self.alpha, beta=1, gamm=1.0, unseen=self.unseen_classes)
    ...
    def forward(self, labels, lab_att, imgs, mask_ratio=0.25):
        ...
        latent, mask, ids_restore, cls_out, hash_out, latent_attr, cls_out_attr, hash_out_attr = self.forward_encoder(
            imgs, attr, lab_att, labels, mask_ratio)
...
def mae_vit_base_patch16_dec512d8b(**kwargs):
    with open('/mnt/f88fa63a-2225-40fb-9afa-99a7c125ae28/jy/mae_jy_cub/word2vec/CUB_attribute.pkl', 'rb') as f:
